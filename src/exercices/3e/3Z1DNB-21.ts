...
export default class ExercicePolynesie592015 extends ExerciceBrevetA {
  ...
  private appliquerLesValeurs(
    ...
    correction1 += `$${texNombre(aireTotale / 10000, 2)} \\times ${densiteChevre} = ${miseEnEvidence(texNombre((aireTotale / 10000) * densiteChevre, 2))}$, soit un maximum de ${nombreChevre} chèvres.<br>`
    ...
    correction1 += `Aire totale : $${grandeLongueur} \\times ${petiteLongueur} + ${petiteLongueur}^2 = ${miseEnEvidence(texNombre(aireTotale))}\\text{ m}^2$.`
    const correction2 = `Les ${nombreChevre} chèvres donneront en moyenne par jour : $${nombreChevre} \\times ${texNombre(productionLait, 1)} = ${miseEnEvidence(texNombre(productionLaitTotale, 1))}$ litres de lait.`
