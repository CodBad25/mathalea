...
type AnimationSvgElement = Element & {
  beginElement: () => void
  endElement: () => void
}
type IntervallesAnimation = Record<string, ReturnType<typeof setInterval>[]>
type ReplayAnimationPayload = {
...
type RemiseAZero = (
  ...
  nbToursPremiereRoue: number,
  oneCycle: boolean,
  intervallesId: IntervallesAnimation,
) => void

export default class EngrenagesAnimes extends Exercice {
  declare sup: number
  declare sup2: boolean
  ...
  nouvelleVersion(numeroExercice: number) {
    ...
    const remiseAZeroT = function (
      ...
      try {
        const animRoue = document
          ...
          ?.querySelectorAll<AnimationSvgElement>('[id^=animRoue]')
        if (!animRoue) return
        const compteurRoue = document
          .querySelector(`#containerAnimRoues${numeroExercice}`)
          ?.querySelectorAll<SVGTextElement>('text[id^=compteur]')
        if (!compteurRoue) return
        ...
        compteurRoue.forEach((e, i) => {
          const inter = setInterval(
            () => {
              e.textContent = String(Number.parseInt(e.textContent ?? '0') + 1)
            },
            Number.parseFloat(animRoue[i].getAttribute('dur') ?? '0') * 1000,
          )
          intervallesId[numeroExercice] = [
            ...intervallesId[numeroExercice],
            inter,
        ...
        if (oneCycle) {
          ...
          setTimeout(
            ...
            nbToursPremiereRoue *
              Number.parseFloat(animRoue[0].getAttribute('dur') ?? '0') *
              1000,
          )
        }
      } catch (e) {
    ...
    for (let i = 0, cpt = 0; i < this.nbQuestions && cpt < 50; cpt++) {
      ...
      let roues: ReturnType<typeof engrenages> = []
      let rouesCorr: ReturnType<typeof engrenages> = []
...
function registerReplayAnimation(remiseAZeroT: RemiseAZero) {
  ...
  DomReadyActionElement.registerCallback<ReplayAnimationPayload>(
    ...
    ({ payload }) => {
      ...
      const intervallesId: IntervallesAnimation = {}
