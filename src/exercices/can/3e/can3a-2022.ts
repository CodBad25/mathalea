  ...
  nouvelleVersion() {
    ...
    for (
      let i = 0,
        ...
        texteCorr = '',
        reponse:
          number | string | string[] | Decimal | ReturnType<typeof fraction> =
          0,
        fraction23,
        poly,
        propositions,
      ...
      switch (typeQuestionsDisponibles[i]) {
        ...
        case 9:
          ...
          if (!(a instanceof Decimal) || !(b instanceof Decimal))
            throw new Error('Facteurs décimaux non initialisés')
    ...
    $\\dfrac{${a}}{${a}}\\times\\dfrac{${b}}{${b}}\\times\\dfrac{${d}}{${d}}\\times\\dfrac{${e}}{${f}}=\\dfrac{${e}}{${f}}${simplificationDeFractionAvecEtapes(e, f)}$ .`
      ...
      `
        ...
        case 29: {
          a = choice([-3, -2, -1, 2, 3])
          const multiplicateurs: Array<[string, number]> = [
            ['double', 2],
            ['triple', 3],
          ]
          const multiplicateur = choice(multiplicateurs)
