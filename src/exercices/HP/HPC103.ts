...
export default class NomExercice extends Exercice {
  ...
  nouvelleVersion() {
    ...
    for (
      ...
      const matrices: Matrice[] = [] // vecteur qui stocke les matrices
      const matricesprint: string[] = [] // vecteur qui stocke les matrices écrites en LaTeX
      const nbmatrice = 2
      let n = randint(1, 4)
      let m = randint(1, 4)
      while (n === m && n === 1) {
      ...
      const nblignes: number[] = [] // vecteur qui stocke le nombre de lignes de chaque matrice
      nblignes.push(n)
      const nbcolonnes: number[] = [] // vecteur qui stocke le nombre de colonnes de chaque matrice
      nbcolonnes.push(m)
      ...
      nbcolonnes.push(m)
      const texteligne: string[] = [] // texte pour la correction
      const textecolonne: string[] = [] // texte pour la correction
      ...
      if (
        this.questionJamaisPosee(
          i,
          matrices.map((item) => item.toString()).join('|'),
        )
      ) {
