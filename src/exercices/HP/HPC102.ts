  ...
  nouvelleVersion() {
    ...
    for (
      let i = 0,
        texte = '',
        texteCorr = '',
        variables: { a: number; b: number; mu: number; sigma: number } = {
          a: 0,
          b: 0,
          mu: 0,
          sigma: 1,
        ...
        case 'Nmusigmaintervallecentre':
          {
            ...
            variables = { a, b: a, mu: mu2, sigma: sigma2 }
            gaussienne = (x: number) =>
              (1 / variables.sigma / Math.sqrt(2 * Math.PI)) *
              Math.exp(-((x - variables.mu) ** 2) / 2 / variables.sigma ** 2)
            r = repere({
