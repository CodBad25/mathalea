...
export default class AutoQ11ANns2026 extends ExerciceQcmA {
  private appliquerLesValeurs(dx: number, dy: number, sx: number, nbSols: number): void {
...
this.enonce = `${deuxColonnes(`Une fonction $h$ définie sur $[${domaineMin}\\,;\\,${domaineMax}]$ est représentée ci-contre.<br>
    ${context.isHtml ? '<br><br><br><br><br><br><br><br><br><br><br><br>' : '\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\'}
    L'équation $h(x) = ${k}$ a pour ensemble solution :<br>`,`${figure}`)}`
    ...
    const strSolsList = nbSols === 2 ? `$${sols[0]}$ et $${sols[1]}$` : `$${sols[0]}$, $${sols[1]}$ et $${sols[2]}$`
