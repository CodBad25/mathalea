...
```sh
pnpm --pm-on-fail=ignore exec playwright install
```
...
```sh
pnpm --pm-on-fail=ignore test:unit
pnpm --pm-on-fail=ignore test:src
pnpm --pm-on-fail=ignore check
```
...
```sh
CI=true PLAYWRIGHT_SERVER_PORT=5173 pnpm --pm-on-fail=ignore test:e2e:views
CI=true PLAYWRIGHT_SERVER_PORT=5173 pnpm --pm-on-fail=ignore test:e2e:interactivity
CI=true PLAYWRIGHT_SERVER_PORT=5173 pnpm --pm-on-fail=ignore test:e2e:consistency
```

Pour vérifier les erreurs console sur un exercice ou un niveau précis, utilisez `FILE`:

```sh
CI=true PLAYWRIGHT_SERVER_PORT=5173 FILE=src/exercices/6e/6C10.ts pnpm --pm-on-fail=ignore test:exo:file
```
...
```sh
CI=true PLAYWRIGHT_SERVER_PORT=5173 pnpm --pm-on-fail=ignore test:e2e:pdfexports
CI=true PLAYWRIGHT_SERVER_PORT=5173 pnpm --pm-on-fail=ignore test:e2e:latex_compile
CI=true PLAYWRIGHT_SERVER_PORT=5173 pnpm --pm-on-fail=ignore test:e2e:latex_breaks
```
...
pnpm --pm-on-fail=ignore lintSvelte
```
...
```sh
pnpm --pm-on-fail=ignore makeJson
pnpm --pm-on-fail=ignore exec vite --port 5174
```
...
```sh
CI=true PLAYWRIGHT_SERVER_PORT=5174 pnpm --pm-on-fail=ignore test:e2e:views
```
...
Si l'erreur persiste, copiez le message complet, la commande exacte, votre branche, `node --version` et `pnpm --pm-on-fail=ignore --version` dans votre demande d'aide. Cela permet de distinguer un problème d'environnement, de dépendance, de branche ou de code.
