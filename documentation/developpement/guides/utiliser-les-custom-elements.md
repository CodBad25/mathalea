# Utiliser les custom elements dans un exercice

Ce guide donne des recettes simples pour ajouter une interaction dans un nouvel exercice. Il complète [Rendre un exercice interactif](rendre-un-exercice-interactif.md), avec moins de détails techniques et plus de blocs à recopier.
...
```bash
CHANGED_FILES=src/exercices/6e/6M2B-1.ts pnpm --pm-on-fail=ignore exec vitest run tests/integration/interactivity_all.test.ts
```
